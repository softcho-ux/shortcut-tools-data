...
const promptData = {
  claude_code: {
    ...
    categories: [
      ...
      { name: 'Hook Events', shortcuts: [
        { keys: 'SessionStart', desc: 'When a session begins or resumes', usage: 'Matcher filters how the session started: startup, resume, clear, compact, fork.' },
        { keys: 'Setup', desc: 'When you start Claude Code with --init-only, or with --init or --maintenance in -p mode', usage: 'For one-time preparation in CI or scripts. Matcher values: init, maintenance.' },
        { keys: 'UserPromptSubmit', desc: 'When you submit a prompt, before Claude processes it', usage: 'No matcher support - fires on every prompt.' },
        { keys: 'UserPromptExpansion', desc: 'When a user-typed command expands into a prompt, before it reaches Claude', usage: 'Can block the expansion. Matcher filters by your skill or command names.' },
        { keys: 'PreToolUse', desc: 'Before a tool call executes; can block it', usage: 'Matcher filters by tool name, e.g. Bash, Edit|Write, mcp__.*' },
        { keys: 'PermissionRequest', desc: 'When a tool call needs a permission decision', usage: 'Matcher filters by tool name, same values as PreToolUse.' },
        { keys: 'PermissionDenied', desc: 'When auto mode denies a tool call, including denials without a classifier verdict', usage: 'hookSpecificOutput.retry: true tells the model it may retry; ignored when the classifier produced no verdict.' },
        { keys: 'PostToolUse', desc: 'After a tool call succeeds', usage: 'Matcher filters by tool name.' },
        { keys: 'PostToolUseFailure', desc: 'After a tool call fails', usage: 'Matcher filters by tool name.' },
        { keys: 'PostToolBatch', desc: 'After a full batch of parallel tool calls resolves, before the next model call', usage: 'No matcher support.' },
        { keys: 'Notification', desc: 'When Claude Code sends a notification', usage: 'Matcher filters by notification type, e.g. permission_prompt, idle_prompt, agent_needs_input, agent_completed.' },
        { keys: 'MessageDisplay', desc: 'While assistant message text is displayed', usage: 'Display-only event; no matcher support.' },
        { keys: 'SubagentStart', desc: 'When a subagent is spawned', usage: 'Matcher filters by agent type: general-purpose, Explore, Plan, custom or plugin-scoped agent names.' },
        { keys: 'SubagentStop', desc: 'When a subagent finishes', usage: 'Same matcher values as SubagentStart.' },
        { keys: 'TaskCreated', desc: 'When a task is being created via TaskCreate', usage: 'No matcher support.' },
        { keys: 'TaskCompleted', desc: 'When a task is being marked as completed', usage: 'No matcher support.' },
        { keys: 'Stop', desc: 'When Claude finishes responding', usage: 'No matcher support. Claude Code overrides a Stop hook after it blocks eight times in a row without progress.' },
        { keys: 'StopFailure', desc: 'When the turn ends due to an API error', usage: 'Matcher filters by error type, e.g. rate_limit, overloaded, server_error, max_output_tokens.' },
        { keys: 'TeammateIdle', desc: 'When an agent team teammate is about to go idle', usage: 'No matcher support.' },
        { keys: 'InstructionsLoaded', desc: 'When a CLAUDE.md or .claude/rules/*.md file is loaded into context', usage: 'Fires at session start and on lazy loads. Matcher values include session_start, nested_traversal, include, compact.' },
        { keys: 'ConfigChange', desc: 'When a configuration file changes during a session', usage: 'Matcher filters by source: user_settings, project_settings, local_settings, policy_settings, skills.' },
        { keys: 'CwdChanged', desc: 'When the working directory changes, for example when Claude executes a cd command', usage: 'No matcher support - fires on every directory change.' },
        { keys: 'DirectoryAdded', desc: 'When a working directory is added mid-session via /add-dir or the SDK register_repo_root request', usage: 'Matcher values: slash_command, register_repo_root.' },
        { keys: 'FileChanged', desc: 'When a watched file changes on disk', usage: 'The matcher field specifies literal filenames to watch, e.g. .envrc|.env' },
        { keys: 'WorktreeCreate', desc: 'When a worktree is being created via --worktree, isolation: worktree, or for a background session', usage: 'Replaces default git behavior. No matcher support.' },
        { keys: 'WorktreeRemove', desc: 'When a worktree is being removed at session exit, when a subagent finishes, or when you delete a background session', usage: 'No matcher support.' },
        { keys: 'PreCompact', desc: 'Before context compaction', usage: 'Matcher filters what triggered compaction: manual, auto.' },
        { keys: 'PostCompact', desc: 'After context compaction completes', usage: 'Matcher values: manual, auto.' },
        { keys: 'Elicitation', desc: 'When an MCP server requests user input during a tool call', usage: 'Matcher filters by your configured MCP server names.' },
        { keys: 'ElicitationResult', desc: 'After a user responds to an MCP elicitation, before the response is sent back to the server', usage: 'Same matcher values as Elicitation.' },
        { keys: 'SessionEnd', desc: 'When a session terminates', usage: 'Matcher filters why the session ended: clear, resume, logout, prompt_input_exit, other.' }
      ] }
    ]
  },
  cursor: {
  ...
  openclaw: {
    ...
    categories: [
      ...
      { name: 'Hook Events & CLI', shortcuts: [
        { keys: 'command:new', desc: 'Fires when the /new command is issued', usage: 'Hooks subscribe to a specific key from the event table, declared in HOOK.md as events: [command:new].' },
        { keys: 'command:reset', desc: 'Fires when the /reset command is issued', usage: 'Specific keys are preferred over the bare family name to reduce overhead.' },
        { keys: 'command:stop', desc: 'Fires when the /stop command is issued', usage: 'Handlers should filter events early and return immediately if not relevant.' },
        { keys: 'command', desc: 'Any command event - general listener for the whole family', usage: 'Bare family names (command, session, agent, gateway, message) receive every action in that family.' },
        { keys: 'session:auto-reset', desc: 'A daily or idle reset replaces the current session', usage: 'OpenClaw core emits nothing outside the documented table; unknown names leave a hook silently dead.' },
        { keys: 'session:compact:before', desc: 'Before compaction summarizes history', usage: 'The hook loader logs a warning for typo event names such as command:nwe.' },
        { keys: 'session:compact:after', desc: 'After compaction completes', usage: 'Pair with session:compact:before to bracket a compaction.' },
        { keys: 'session:patch', desc: 'When session properties are modified', usage: 'Part of the session event family.' },
        { keys: 'agent:bootstrap', desc: 'Before workspace bootstrap files are injected', usage: 'The only documented agent-family event key.' },
        { keys: 'gateway:startup', desc: 'After channels start and hooks are loaded', usage: 'Restarting the gateway process reloads hooks.' },
        { keys: 'gateway:shutdown', desc: 'When gateway shutdown begins', usage: 'Keep handlers fast - hooks run during command processing.' },
        { keys: 'gateway:pre-restart', desc: 'Before an expected gateway restart', usage: 'Distinct from gateway:shutdown, which covers full shutdown.' },
        { keys: 'message:received', desc: 'Inbound message from any channel', usage: 'Part of the message event family.' },
        { keys: 'message:transcribed', desc: 'After audio transcription completes', usage: 'Fires only for audio messages that go through transcription.' },
        { keys: 'message:preprocessed', desc: 'After media and link preprocessing completes or is skipped', usage: 'Fires either way, so handlers see every inbound message once preprocessing resolves.' },
        { keys: 'message:sent', desc: 'Outbound send attempted', usage: 'context.success carries the result of the send attempt.' },
        { keys: 'openclaw hooks list', desc: 'List all discovered hooks', usage: 'Add --eligible, --verbose, or --json. A hook must show here to be enabled.' },
        { keys: 'openclaw hooks info <hook-name>', desc: 'Show detailed info about a hook', usage: 'Flags unknown event names, so a hook that never runs is diagnosable.' },
        { keys: 'openclaw hooks check', desc: 'Show the eligibility summary', usage: 'Eligibility fails on missing binaries (PATH), environment variables, config values, or OS compatibility.' },
        { keys: 'openclaw hooks enable <hook-name>', desc: 'Enable a hook', usage: 'A hook needs HOOK.md plus a handler file in its directory, e.g. ~/.openclaw/hooks/my-hook/.' },
        { keys: 'openclaw hooks disable <hook-name>', desc: 'Disable a hook', usage: 'Per-hook state lives under hooks.internal.entries in the OpenClaw config; extra directories via hooks.internal.load.extraDirs.' }
      ] },
      { name: 'Channel, Plugin & Runtime Commands', shortcuts: [
        { keys: '/exec host=<auto|sandbox|gateway|node> security=<deny|allowlist|full> ask=<off|on-miss|always> node=<id>', desc: 'Show or set exec defaults', usage: 'Controls where and how shell execution runs for the session.' },
        { keys: '/goal [status|start|edit|pause|resume|complete|block|clear] ...', desc: 'Manage the current session\'s durable goal', usage: 'Goal state persists across the session lifecycle.' },
        { keys: '/acp spawn|cancel|steer|close|sessions|status|set-mode|set|cwd|permissions|timeout|model|reset-options|doctor|install|help', desc: 'Manage ACP sessions and runtime options', usage: 'Runtime controls require external owner or intern privileges.' },
        { keys: '/tts on|off|status|chat|latest|provider|limit|summary|audio|help', desc: 'Control text-to-speech output', usage: 'Provider, limits, and summary behavior are configurable per session.' },
        { keys: '/dreaming [on|off|status|help]', desc: 'Toggle memory dreaming', usage: 'Owner or Gateway admin only.' },
        { keys: '/pair [qr|status|pending|approve|cleanup|notify]', desc: 'Manage device pairing', usage: 'Covers QR pairing, pending approvals, and cleanup.' },
        { keys: '/voice status|list|set <voiceId>', desc: 'Manage Talk voice configuration', usage: 'Discord native command name: /talkvoice.' },
        { keys: '/card ...', desc: 'Send LINE rich card presets', usage: 'LINE channel only.' },
        { keys: '/dock-discord', desc: 'Switch the active session\'s reply route to Discord', usage: 'Alias: /dock_discord. Requires session.identityLinks; source sender and target peer must share an identity group.' },
        { keys: '/dock-mattermost', desc: 'Switch the active session\'s reply route to Mattermost', usage: 'Alias: /dock_mattermost. Same identity-link requirement as other dock commands.' },
        { keys: '/dock-slack', desc: 'Switch the active session\'s reply route to Slack', usage: 'Alias: /dock_slack. Same identity-link requirement as other dock commands.' },
        { keys: '/dock-telegram', desc: 'Switch the active session\'s reply route to Telegram', usage: 'Alias: /dock_telegram. Same identity-link requirement as other dock commands.' },
      ]},
    ]
  },
  nous_hermes: {
    ...
    categories: [
      ...
      { name: 'Session (CLI)', shortcuts: [
        { keys: '/new [name] (alias: /reset )', desc: 'Start a new session (fresh session ID + history).', usage: 'Optional [name] sets the initial session title — e.g. /new my-experiment opens a fresh session already titled my-experiment so it\'s easy to find later with /resume or /sessions . Append now , --yes , or -y to skip the confirmation modal — e.g. /reset now , /new --yes my-experimen' },
        { keys: '/clear', desc: 'Clear screen and start a new session', usage: '' },
        { keys: '/history', desc: 'Show conversation history (respects /timestamps )', usage: '' },
        { keys: '/save', desc: 'Save the current conversation', usage: '' },
        { keys: '/prompt (alias: /compose )', desc: 'Compose your next prompt in $EDITOR (markdown) instead of the inline input — useful for long, multi-line, or carefully-formatted prompts.', usage: '' },
        { keys: '/retry', desc: 'Retry the last message (resend to agent)', usage: '' },
        { keys: '/undo', desc: 'Remove the last user/assistant exchange', usage: '' },
        { keys: '/title', desc: 'Set a title for the current session (usage: /title My Session Name)', usage: '' },
        ...
        { keys: '/rollback', desc: 'List or restore filesystem checkpoints (usage: /rollback [number])', usage: '' },
        { keys: '/diff [staged|all|session] [--stat] [path...]', desc: 'Show git changes in the working directory.', usage: 'Default: unstaged changes plus untracked files. staged shows what\'s staged for commit, all everything since HEAD, and session the cumulative diff of everything Hermes changed here (from the earliest retained checkpoint baseline — requires checkpoints to be enabled; complements /r' },
        ...
        { keys: '/stop', desc: 'Kill all running background processes', usage: '' },
        { keys: '/queue <prompt> (alias: /q )', desc: 'Queue a prompt for the next turn (doesn\'t interrupt the current agent response).', usage: '' },
        ...
        { keys: '/resume [name]', desc: 'Resume a previously-named session', usage: '' },
        ...
        { keys: '/egress [status]', desc: 'Show Docker egress proxy status — enabled/configured/running state, credential source, token mappings, uncovered providers, and next remediation step.', usage: 'Works in CLI, TUI, Desktop chat, and messaging gateway.' },
        { keys: '/redraw', desc: 'Force a full UI repaint (recovers from terminal drift after tmux resize, mouse selection artifacts, etc.)', usage: '' },
        { keys: '/status', desc: 'Show session info — model, provider, profile, session ID, working directory, title, created/updated timestamps, token totals, agent-running state — followed by a local Session reca', usage: 'The recap is computed locally from the in-memory conversation; no LLM call, no prompt-cache impact.' },
        { keys: '/context [all] (alias: /ctx )', desc: 'Visual context-window breakdown.', usage: 'On the CLI/TUI: a 5×20 glyph block grid (each cell ≈ 1% of the model window) plus an estimated per-category table — system prompt, tool definitions, rules, skills index, MCP, subagents, memory, conversation — versus free space. On messaging platforms: a usage gauge with auto-comp' },
        { keys: '/agents (alias: /tasks )', desc: 'Show active agents and running tasks across the current session.', usage: '' },
        ...
        { keys: '/branch [name] (alias: /fork )', desc: 'Branch the current session (explore a different path)', usage: '' },
        { keys: '/worktree [new [name]|list]', desc: 'CLI only.', usage: 'Inspect or create isolated git worktrees mid-session (inspired by Copilot CLI\'s /worktree new ). Bare /worktree shows the active worktree; /worktree list lists the repo\'s worktrees; /worktree new [name] creates a worktree under .worktrees/ (branched from the freshly-fetched remot' },
        { keys: '/handoff <platform>', desc: 'CLI only.', usage: 'Hand the current session off to a messaging platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix). The gateway picks it up immediately, creates a fresh thread on platforms that support threads (Telegram topics, Discord text-channel threads, Slack message-anchored threads)' },
      ...
      { name: 'Configuration (CLI)', shortcuts: [
        { keys: '/config', desc: 'Show current configuration', usage: '' },
        { keys: '/model [model-name]', desc: 'Show or change the current model.', usage: 'Supports: /model claude-sonnet-4 , /model provider:model (switch providers), /model custom:model (custom endpoint), /model custom:name:model (named custom provider), /model custom (auto-detect from endpoint), and user-defined aliases ( /model fav , /model grok — see Custom model ' },
        { keys: '/codex-runtime [auto|codex_app_server|on|off]', desc: 'Toggle the optional Codex app-server runtime for OpenAI/Codex models.', usage: 'auto (default) uses Hermes\' standard chat completions; codex_app_server hands turns to a codex app-server subprocess for native shell, apply_patch, ChatGPT subscription auth, and migrated Codex plugins. Effective on next session.' },
        ...
        { keys: '/focus [on|off|status]', desc: 'Toggle focus view — a display-only reduced-output mode showing just your prompt and the final response.', usage: 'Composes with /verbose : turning it on snaps tool progress to off and remembers your previous mode, and /focus off restores it. Each turn ends with a dim recovery line ( ⋯ 7 tool lines hidden · /focus off to show ) and a persistent ◉ focus badge sits in the status bar so you alwa' },
        ...
        { keys: '/reasoning [level|show|hide|full|clamp] [--global]', desc: 'Manage reasoning effort and display.', usage: 'Levels include none / minimal / low / medium / high / xhigh / max / ultra . show / hide (or on / off ) toggle reasoning display; full and clamp adjust how reasoning is shown. --global persists effort to config.' },
        { keys: '/skin', desc: 'Show or change the display skin/theme', usage: '' },
        { keys: '/export [profile] [-o out.tar.gz]', desc: 'CLI only.', usage: 'Pack a profile into a shareable .tar.gz — skills, memory, persona, crons, plugins, settings, and (from the desktop) themes and layout. Credentials ( auth.json , .env ) are stripped. Defaults to the active profile and <name>.tar.gz in the current directory. Same archive as hermes ' },
        { keys: '/import <archive.tar.gz> [--name <name>]', desc: 'CLI only.', usage: 'Install a profile archive as a new profile, inferring the name from the archive unless --name is given. Refuses to overwrite an existing profile and cannot import as default . Creates a shell wrapper when the name is free. See Export and import a profile file .' },
        { keys: '/statusbar (alias: /sb )', desc: 'Toggle the context/model status bar on or off', usage: '' },
        { keys: '/battery [on|off|status]', desc: 'Toggle a color-coded battery read-out as the first status-bar element (off by default; no-op without a battery).', usage: '' },
        ...
        { keys: '/yolo', desc: 'Toggle YOLO mode — skip all dangerous command approval prompts.', usage: '' },
        { keys: '/approvals [manual|smart|off]', desc: 'Show or set the persistent dangerous-command approval mode.', usage: '' },
        { keys: '/footer [on|off|status]', desc: 'Toggle the gateway runtime-metadata footer on final replies (shows model, context %, and cwd).', usage: '' },
        { keys: '/busy [queue|steer|interrupt|status]', desc: 'CLI-only: control what pressing Enter does while Hermes is working — queue the new message, steer mid-turn, or interrupt immediately.', usage: '' },
        { keys: '/indicator [kaomoji|emoji|unicode|ascii]', desc: 'CLI-only: pick the TUI busy-indicator style.', usage: '' },
        { keys: '/timestamps [on|off|status]', desc: 'CLI-only: toggle [HH:MM] timestamps on messages and in /history .', usage: '' },
        { keys: '/wake [on|off|status]', desc: 'CLI-only: toggle the "Hey Hermes" wake word listener.', usage: '' },
      ]},
      { name: 'Tools & Skills (CLI)', shortcuts: [
        ...
        { keys: '/toolsets', desc: 'List available toolsets', usage: '' },
        { keys: '/browser [connect|disconnect|status]', desc: 'Manage a local Chromium-family CDP connection.', usage: 'connect attaches browser tools to a running Chrome, Brave, Chromium, or Edge instance (default: http://127.0.0.1:9222 ). disconnect detaches. status shows current connection. Auto-launches a supported Chromium-family browser if no debugger is detected.' },
        ...
        { keys: '/learn <what to learn from>', desc: 'Distill a reusable skill from anything you describe — a directory, a URL, the workflow you just walked the agent through, or pasted notes.', usage: 'Open-ended: the agent gathers the sources with its own tools and authors a SKILL.md following the house authoring standards. Works in the CLI, the messaging gateway, the TUI, and the dashboard Skills page.' },
        { keys: '/init [notes]', desc: 'Generate or update AGENTS.md project instructions from a repo scan (port of Codex /init ).', usage: 'The agent inspects manifests, layout, and toolchain configs with its read-only tools, then writes a concise AGENTS.md — or, if one exists, merge-updates it preserving your content. Optional notes steer the emphasis. Works in the CLI, the messaging gateway, and the TUI.' },
        { keys: '/cron', desc: 'Manage scheduled tasks (list, add/create, edit, pause, resume, run, remove)', usage: '' },
        { keys: '/suggestions [accept|dismiss N|catalog|clear] (alias: /suggest )', desc: 'Review suggested automations.', usage: 'Use /suggestions to list pending suggestions, /suggestions accept <id> to create the proposed automation, /suggestions dismiss <id> to reject one, /suggestions catalog to add curated starter automations, and /suggestions clear to clear resolved suggestion records. Accepted jobs p' },
        ...
        { keys: '/kanban <action>', desc: 'Drive the multi-profile, multi-project collaboration board without leaving chat.', usage: 'Full hermes kanban surface is available: /kanban list , /kanban show t_abc , /kanban create "title" --assignee X , /kanban comment t_abc "text" , /kanban unblock t_abc , /kanban dispatch , etc. Multi-board support included: /kanban boards list , /kanban boards create <slug> , /ka' },
        { keys: '/reload-mcp (alias: /reload_mcp )', desc: 'Reload MCP servers from config.yaml', usage: '' },
        { keys: '/reload-skills (alias: /reload_skills )', desc: 'Re-scan ~/.hermes/skills/ for newly installed or removed skills', usage: '' },
        { keys: '/reload', desc: 'Reload .env variables into the running session (picks up new API keys without restarting)', usage: '' },
        { keys: '/plugins', desc: 'List installed plugins and their status', usage: '' },
      ...
      { name: 'Info & Account (CLI)', shortcuts: [
        ...
        { keys: '/palette', desc: 'Open the fuzzy command palette (also Ctrl+P ) — type to filter all commands + skills, ↑/↓ to move, Enter to insert the selected command into the composer (never auto-runs), Esc to ', usage: 'Matching is ranked by command name first, so a short query stays precise.' },
        { keys: '/version', desc: 'Show Hermes Agent version, build, and environment info.', usage: '' },
        { keys: '/whoami', desc: 'Show your slash command access level (admin / user).', usage: '' },
        { keys: '/usage', desc: 'Show token usage, cost breakdown, session duration, and — when available from the active provider — an Account limits section with remaining quota / credits / plan usage pulled liv', usage: '' },
        { keys: '/topup', desc: 'Show your Nous balance and manage billing on the portal (replaces the old /credits and /billing commands).', usage: '' },
        { keys: '/subscription (alias: /upgrade )', desc: 'CLI only.', usage: 'View your Nous plan and change it in the browser.' },
        { keys: '/insights', desc: 'Show usage insights and analytics (last 30 days)', usage: '' },
        { keys: '/update', desc: 'Update Hermes Agent to the latest version.', usage: '' },
        { keys: '/platforms (alias: /gateway )', desc: 'Show gateway/messaging platform status (CLI-only summary view).', usage: '' },
        { keys: '/paste', desc: 'Attach a clipboard image', usage: '' },
        { keys: '/copy [number]', desc: 'Copy the last assistant response to clipboard (or the Nth-from-last with a number).', usage: 'CLI-only.' },
        { keys: '/image <path>', desc: 'Attach a local image file for your next prompt.', usage: '' },
        { keys: '/debug', desc: 'Upload debug report (system info + logs) and get shareable links.', usage: 'Also available in messaging.' },
        { keys: '/profile', desc: 'Show active profile name and home directory', usage: '' },
      ]},
      { name: 'Exit (CLI)', shortcuts: [
        { keys: '/quit', desc: 'Exit the CLI (also: /exit ).', usage: '' },
      ]},
      { name: 'Messaging-Only Commands', shortcuts: [
        { keys: '/start', desc: 'Platform-protocol command.', usage: 'Many chat platforms (Telegram, Discord, …) send /start automatically the first time a user opens a bot conversation. Hermes acknowledges the ping silently — no agent reply, no session burn — so first-contact handshakes don\'t waste a turn. You can also send it explicitly to confir' },
        { keys: '/sethome (alias: /set-home )', desc: 'Mark the current chat as the platform home channel for deliveries.', usage: '' },
        { keys: '/topic [off|help|session-id]', desc: 'Telegram DM only.', usage: 'Manage user-managed multi-session topic mode. /topic enables it or shows status; /topic off disables it and clears bindings; /topic help shows usage; /topic <session-id> inside a topic restores a previous session. See Multi-session DM mode .' },
        { keys: '/platform <list|pause|resume> [name]', desc: 'Operate a running gateway platform right from chat.', usage: '/platform list shows every adapter and its state (running, paused-by-breaker, manually-paused); /platform pause <name> stops dispatching new messages to that adapter without unloading it; /platform resume <name> re-enables it and clears a tripped circuit breaker once the upstream' },
        { keys: '/commands [page]', desc: 'Browse all commands and skills (paginated).', usage: '' },
        ...
        { keys: '/deny', desc: 'Reject a pending dangerous command.', usage: '' },
        ...
        { keys: '/<skill-name>', desc: 'Invoke any installed skill by name.', usage: '' },
      ]},
]
  },
  mcp: {
