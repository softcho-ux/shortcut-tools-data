...
const promptData = {
  ...
  openclaw: {
    name: 'OpenClaw',
    icon: 'fas fa-paw',
    source: { name: 'OpenClaw Docs - Slash commands', url: 'https://docs.openclaw.ai/tools/slash-commands' },
    categories: [
      { name: 'Session & Run Commands', shortcuts: [
        { keys: '/new [model]', desc: 'Archive the current session and start a fresh one', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/reset [soft [message]]', desc: 'Reset the current session in place', usage: 'soft keeps the transcript, drops reused CLI backend session ids, and reruns startup' },
        { keys: '/name <title>', desc: 'Name or rename the current session', usage: 'Omit the title to see the current name and a suggestion' },
        { keys: '/compact [instructions]', desc: 'Compact the session context', usage: 'See Compaction' },
        { keys: '/stop', desc: 'Abort the current run', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/session idle <duration|off>', desc: 'Manage thread-binding idle expiry', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/session max-age <duration|off>', desc: 'Manage thread-binding max-age expiry', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/export-session [path]', desc: 'Owner-only', usage: 'Export the current session to HTML inside the workspace. Alias: /export' },
        { keys: '/export-trajectory [path]', desc: 'Export a JSONL trajectory bundle for the current session', usage: 'Alias: /trajectory' }
      ] },
      { name: 'Model & Run Controls', shortcuts: [
        { keys: '/think <level|default>', desc: 'Set the thinking level or clear the session override', usage: 'Aliases: /thinking, /t' },
        { keys: '/verbose on|off|full', desc: 'Toggle verbose output', usage: 'Alias: /v' },
        { keys: '/trace on|off', desc: 'Toggle plugin trace output for the current session', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/fast [status|auto|on|off|default]', desc: 'Show, set, or clear fast mode', usage: 'Slash command in any connected chat surface or the CLI.' },
        ...
        { keys: '/exec host=<auto|sandbox|gateway|node> security=<deny|allowlist|full> ask=<off|on-miss|always> node=<id>', desc: 'Show or set exec defaults', usage: 'Slash command in any connected chat surface or the CLI.' },
        ...
        { keys: '/models [provider] [page] [limit=<n>|all]', desc: 'List configured/auth-available providers or models', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/queue <mode>', desc: 'Manage active-run queue behavior', usage: 'See Queue and Queue steering' },
        { keys: '/steer <message>', desc: 'Inject guidance into the active run', usage: 'Alias: /tell. See Steer' }
      ] },
      { name: 'Discovery & Status Commands', shortcuts: [
        { keys: '/help', desc: 'Show the short help summary', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/commands', desc: 'Show the generated command catalog', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/tools [compact|verbose]', desc: 'Show what the current agent can use right now', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/status', desc: 'Show execution/runtime status, Gateway and system uptime, plugin health, plus provider usage/quota', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/status plugins', desc: 'Show detailed plugin health: load errors, quarantines, channel plugin failures, dependency issues, compatibility notices', usage: 'Requires commands.plugins: true' },
        { keys: '/goal [status|start|edit|pause|resume|complete|block|clear] ...', desc: 'Manage the current session\'s durable goal', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/diagnostics [note]', desc: 'Owner-only support-report flow', usage: 'Asks for exec approval every time' },
        { keys: '/openclaw <request>', desc: 'Run the OpenClaw setup and repair helper from an owner DM', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/tasks', desc: 'List active/recent background tasks for the current session', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/context [list|detail|map|json]', desc: 'Explain how context is assembled', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/whoami', desc: 'Show your sender id', usage: 'Alias: /id' },
        { keys: '/usage off|tokens|full|reset|cost', desc: 'Control the per-response usage footer (reset/inherit/clear/default clears the session override to re-inherit the...', usage: 'configured default) or print a local cost summary' }
      ] },
      { name: 'Skills, Allowlists & Approvals', shortcuts: [
        { keys: '/skill <name> [input]', desc: 'Run a skill by name', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/learn [request]', desc: 'Draft one reviewable skill from the current conversation or named sources through Skill Workshop', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/loop [interval] <prompt>', desc: 'Owner-only', usage: 'Repeat a prompt in this conversation; omit the interval for self-paced checks' },
        { keys: '/loop status', desc: 'Owner-only', usage: 'List loops bound to this conversation' },
        { keys: '/loop stop [name]', desc: 'Owner-only', usage: 'Stop matching loops bound to this conversation' },
        { keys: '/allowlist [list|add|remove] ...', desc: 'Manage allowlist entries', usage: 'Text-only' },
        { keys: '/approve <id> <decision>', desc: 'Resolve exec or plugin approval prompts', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/btw <question>', desc: 'Ask a side question without changing session context', usage: 'Alias: /side. See BTW' }
      ] },
      { name: 'Subagent & ACP Commands', shortcuts: [
        { keys: '/subagents list|log|info', desc: 'Inspect sub-agent runs for the current session', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/acp spawn|cancel|steer|close|sessions|status|set-mode|set|cwd|permissions|timeout|model|reset-options|doctor|install|help', desc: 'Manage ACP sessions and runtime options', usage: 'Runtime controls require external owner or internal Gateway admin identity' },
        { keys: '/focus <target>', desc: 'Bind the current Discord thread or Telegram topic to a session target', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/unfocus', desc: 'Remove the current thread binding', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/agents', desc: 'List thread-bound agents for the current session', usage: 'Slash command in any connected chat surface or the CLI.' }
      ] },
      { name: 'Voice, TTS & Channel Commands', shortcuts: [
        { keys: '/tts on|off|status|chat|latest|provider|limit|summary|audio|help', desc: 'Control TTS', usage: 'See TTS' },
        { keys: '/activation mention|always', desc: 'Set group activation mode', usage: 'Slash command in any connected chat surface or the CLI.' },
        { keys: '/bash <command>', desc: 'Run a host shell command', usage: 'Alias: ! <command>. Requires commands.bash: true' },
        { keys: '/dreaming [on|off|status|help]', desc: 'Toggle memory dreaming (owner or Gateway admin)', usage: 'See Dreaming' },
        { keys: '/pair [qr|status|pending|approve|cleanup|notify]', desc: 'Manage device pairing', usage: 'See Pairing' },
        { keys: '/card ...', desc: 'Send LINE rich card presets', usage: 'See LINE' },
        { keys: '/codex <action> ...', desc: 'Bind, steer, and inspect the Codex app-server harness (status, threads, resume, model, fast, permissions, compact,...', usage: 'review, mcp, skills, and more) See Codex harness' }
      ] },
      { name: 'Dock & Skill Commands', shortcuts: [
        { keys: '/dock-discord', desc: 'Bind the session to a Discord surface', usage: 'Alias: /dock_discord. Part of the dock command set for routing replies.' },
        { keys: '/dock-mattermost', desc: 'Bind the session to a Mattermost surface', usage: 'Alias: /dock_mattermost.' },
        { keys: '/dock-slack', desc: 'Bind the session to a Slack surface', usage: 'Alias: /dock_slack.' },
        { keys: '/dock-telegram', desc: 'Bind the session to a Telegram surface', usage: 'Alias: /dock_telegram.' },
        { keys: '/skill <name> [input]', desc: 'Generic entrypoint for invoking any installed skill', usage: 'Always available even when a skill does not register its own command.' }
      ] }
    ]
  },
  hermes: {
    name: 'Nous Hermes (Agent & Models)',
    icon: 'fas fa-scroll',
    source: { name: 'Hermes Agent Docs - Slash commands reference', url: 'https://hermes-agent.nousresearch.com/docs/reference/slash-commands' },
    categories: [
      { name: 'Session Commands', shortcuts: [
        { keys: '/new [name] (alias: /reset)', desc: 'Start a new session (fresh session ID + history)', usage: 'Optional [name] sets the initial session title — e.g. /new my-experiment opens a fresh session already titled my-experiment so it\'s easy to find later with /resume or /sessions. Append now, --yes,...' },
        { keys: '/clear', desc: 'Clear screen and start a new session', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/history', desc: 'Show conversation history (respects /timestamps)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/save', desc: 'Save the current conversation', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/prompt (alias: /compose)', desc: 'Compose your next prompt in $EDITOR (markdown) instead of the inline input — useful for long, multi-line, or...', usage: 'carefully-formatted prompts' },
        { keys: '/retry', desc: 'Retry the last message (resend to agent)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/undo', desc: 'Remove the last user/assistant exchange', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/title', desc: 'Set a title for the current session (usage: /title My Session Name)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/compress [here [N] | focus topic]', desc: 'Manually compress conversation context (flush memories + summarize)', usage: '/compress here [N] summarizes everything except the most recent N exchanges (default 2), kept verbatim — pick your own compression boundary. A focus topic narrows what a full summary preserves.' },
        { keys: '/rollback', desc: 'List or restore filesystem checkpoints (usage: /rollback [number])', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/diff [staged|all|session] [--stat] [path...]', desc: 'Show git changes in the working directory', usage: 'Default: unstaged changes plus untracked files. staged shows what\'s staged for commit, all everything since HEAD, and session the cumulative diff of everything Hermes changed here (from the...' },
        { keys: '/snapshot [create|restore <id>|prune] (alias: /snap)', desc: 'Create or restore state snapshots of Hermes config/state', usage: 'create [label] saves a snapshot, restore <id> reverts to it, prune [N] removes old snapshots, or list all with no args.' },
        { keys: '/stop', desc: 'Kill all running background processes', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/queue <prompt> (alias: /q)', desc: 'Queue a prompt for the next turn (doesn\'t interrupt the current agent response)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/steer <prompt>', desc: 'Inject a mid-run note that arrives at the agent after the next tool call — no interrupt, no new user turn', usage: 'The text is appended to the last tool result\'s content once the current tool completes, giving the agent new context without breaking the current tool-calling loop. Use this to nudge direction...' },
        { keys: '/goal <text>', desc: 'Set a standing goal Hermes works toward across turns — our take on the Ralph loop', usage: 'After each turn an auxiliary judge model decides whether the goal is done; if not, Hermes auto-continues. Subcommands: /goal status, /goal pause, /goal resume, /goal clear. Budget defaults to 20...' },
        { keys: '/subgoal <text>', desc: 'Append a user-supplied criterion to the active goal mid-loop', usage: 'The continuation prompt surfaces all subgoals to the agent verbatim, and the judge factors them into its DONE/CONTINUE verdict — so the goal isn\'t marked done until the original goal and every...' },
        { keys: '/heartbeat every <interval> <prompt> (alias: /hb)', desc: 'Set a recurring prompt that re-enters this session as a normal user turn whenever it\'s idle and the interval has...', usage: 'elapsed (min 60s; missed ticks coalesce) Subcommands: /heartbeat status, /heartbeat pause, /heartbeat resume, /heartbeat clear. Session-scoped and in-process — use hermes cron for durable isolated...' },
        { keys: '/refine [focus]', desc: 'Run the background memory/skill self-improvement review now instead of waiting for the automatic post-turn trigger', usage: 'Optional focus text steers the review (e.g. /refine save the deploy workflow as a skill). Runs in a background fork against a conversation snapshot — the live session and prompt cache are...' },
        { keys: '/moa <prompt>', desc: 'Run a single prompt through the default Mixture of Agents preset, then restore your current model', usage: 'One-shot — does not change your session model.' },
        { keys: '/resume [name]', desc: 'Resume a previously-named session', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/sessions (TUI alias: /switch)', desc: 'Classic CLI: browse and resume previous sessions in an interactive picker', usage: 'TUI: open the live session switcher for currently open TUI sessions. Use /sessions new in the TUI to start another live session immediately.' },
        { keys: '/egress [status]', desc: 'Show Docker egress proxy status — enabled/configured/running state, credential source, token mappings, uncovered...', usage: 'providers, and next remediation step Works in CLI, TUI, Desktop chat, and messaging gateway.' },
        { keys: '/redraw', desc: 'Force a full UI repaint (recovers from terminal drift after tmux resize, mouse selection artifacts, etc.)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/status', desc: 'Show session info — model, provider, profile, session ID, working directory, title, created/updated timestamps,...', usage: 'token totals, agent-running state — followed by a local Session recap block (recent user/assistant turn counts, tool result count, top tools used, last few files touched, the latest user prompt,...' },
        { keys: '/context [all] (alias: /ctx)', desc: 'Visual context-window breakdown', usage: 'On the CLI/TUI: a 5×20 glyph block grid (each cell ≈ 1% of the model window) plus an estimated per-category table — system prompt, tool definitions, rules, skills index, MCP, subagents, memory,...' },
        { keys: '/agents (alias: /tasks)', desc: 'Show active agents and running tasks across the current session', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/background <prompt> (alias: /bg, /btw)', desc: 'Run a prompt in a separate background session', usage: 'The agent processes your prompt independently — your current session stays free for other work. Results appear as a panel when the task finishes. See CLI Background Sessions.' },
        { keys: '/branch [name] (alias: /fork)', desc: 'Branch the current session (explore a different path)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/worktree [new [name]|list]', desc: 'CLI only', usage: 'Inspect or create isolated git worktrees mid-session (inspired by Copilot CLI\'s /worktree new). Bare /worktree shows the active worktree; /worktree list lists the repo\'s worktrees; /worktree new...' },
        { keys: '/handoff <platform>', desc: 'CLI only', usage: 'Hand the current session off to a messaging platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix). The gateway picks it up immediately, creates a fresh thread on platforms that support...' },
        { keys: '/journey [list|delete <id>|edit <id>] (aliases: /learning, /memory-graph)', desc: 'Open the learning journey timeline of learned skills + memories', usage: 'Works in the classic CLI, as a TUI overlay, and in the desktop app (Star Map panel). Not available on messaging platforms. See Learning Journey.' }
      ] },
      { name: 'Configuration Commands', shortcuts: [
        { keys: '/config', desc: 'Show current configuration', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/model [model-name]', desc: 'Show or change the current model', usage: 'Supports: /model claude-sonnet-4, /model provider:model (switch providers), /model custom:model (custom endpoint), /model custom:name:model (named custom provider), /model custom (auto-detect from...' },
        { keys: '/codex-runtime [auto|codex_app_server|on|off]', desc: 'Toggle the optional Codex app-server runtime for OpenAI/Codex models', usage: 'auto (default) uses Hermes\' standard chat completions; codex_app_server hands turns to a codex app-server subprocess for native shell, apply_patch, ChatGPT subscription auth, and migrated Codex...' },
        { keys: '/personality', desc: 'Set a predefined personality', usage: '/personality none (or default / neutral) clears the overlay and returns to base behavior.' },
        { keys: '/verbose', desc: 'Cycle tool progress display: off → new → all → verbose', usage: 'Can be enabled for messaging via config.' },
        { keys: '/focus [on|off|status]', desc: 'Toggle focus view — a display-only reduced-output mode showing just your prompt and the final response', usage: 'Composes with /verbose: turning it on snaps tool progress to off and remembers your previous mode, and /focus off restores it. Each turn ends with a dim recovery line (⋯ 7 tool lines hidden ·...' },
        { keys: '/fast [normal|fast|status]', desc: 'Toggle fast mode — OpenAI Priority Processing / Anthropic Fast Mode', usage: 'Options: normal, fast, status.' },
        { keys: '/reasoning [level|show|hide|full|clamp] [--global]', desc: 'Manage reasoning effort and display', usage: 'Levels include none / minimal / low / medium / high / xhigh / max / ultra. show / hide (or on / off) toggle reasoning display; full and clamp adjust how reasoning is shown. --global persists...' },
        { keys: '/skin', desc: 'Show or change the display skin/theme', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/export [profile] [-o out.tar.gz]', desc: 'CLI only', usage: 'Pack a profile into a shareable .tar.gz — skills, memory, persona, crons, plugins, settings, and (from the desktop) themes and layout. Credentials (auth.json, .env) are stripped. Defaults to the...' },
        { keys: '/import <archive.tar.gz> [--name <name>]', desc: 'CLI only', usage: 'Install a profile archive as a new profile, inferring the name from the archive unless --name is given. Refuses to overwrite an existing profile and cannot import as default. Creates a shell...' },
        { keys: '/statusbar (alias: /sb)', desc: 'Toggle the context/model status bar on or off', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/battery [on|off|status]', desc: 'Toggle a color-coded battery read-out as the first status-bar element (off by default; no-op without a battery)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/voice [on|off|tts|status]', desc: 'Toggle CLI voice mode and spoken playback', usage: 'Recording uses voice.record_key (default: Ctrl+B).' },
        { keys: '/yolo', desc: 'Toggle YOLO mode — skip all dangerous command approval prompts', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/approvals [manual|smart|off]', desc: 'Show or set the persistent dangerous-command approval mode', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/footer [on|off|status]', desc: 'Toggle the gateway runtime-metadata footer on final replies (shows model, context %, and cwd)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/busy [queue|steer|interrupt|status]', desc: 'CLI-only: control what pressing Enter does while Hermes is working — queue the new message, steer mid-turn, or...', usage: 'interrupt immediately' },
        { keys: '/indicator [kaomoji|emoji|unicode|ascii]', desc: 'CLI-only: pick the TUI busy-indicator style', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/timestamps [on|off|status]', desc: 'CLI-only: toggle [HH:MM] timestamps on messages and in /history', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/wake [on|off|status]', desc: 'CLI-only: toggle the "Hey Hermes" wake word listener', usage: 'Interactive CLI slash command; most also work over messaging channels.' }
      ] },
      { name: 'Tools & Skills Commands', shortcuts: [
        { keys: '/tools [list|disable|enable] [name...]', desc: 'Manage tools: list available tools, or disable/enable specific tools for the current session', usage: 'Disabling a tool removes it from the agent\'s toolset and triggers a session reset.' },
        { keys: '/toolsets', desc: 'List available toolsets', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/browser [connect|disconnect|status]', desc: 'Manage a local Chromium-family CDP connection', usage: 'connect attaches browser tools to a running Chrome, Brave, Chromium, or Edge instance (default: http://127.0.0.1:9222). disconnect detaches. status shows current connection. Auto-launches a...' },
        { keys: '/skills', desc: 'Search, install, inspect, or manage skills from online registries', usage: 'Also the review surface for the skill write-approval gate: /skills pending, /skills diff <id>, /skills approve <id>, /skills reject <id>, /skills approval on|off. See Gating agent skill writes.' },
        { keys: '/memory [pending|approve|reject|approval]', desc: 'Review pending memory writes staged by the write-approval gate (memory.write_approval) and toggle the gate', usage: 'See Controlling memory writes.' },
        { keys: '/bundles', desc: 'List configured skill bundles — /<name> slash aliases that preload several skills at once', usage: 'Configure under bundles: in ~/.hermes/config.yaml. See Skill Bundles.' },
        { keys: '/learn <what to learn from>', desc: 'Distill a reusable skill from anything you describe — a directory, a URL, the workflow you just walked the agent...', usage: 'through, or pasted notes Open-ended: the agent gathers the sources with its own tools and authors a SKILL.md following the house authoring standards. Works in the CLI, the messaging gateway, the...' },
        { keys: '/init [notes]', desc: 'Generate or update AGENTS.md project instructions from a repo scan (port of Codex /init)', usage: 'The agent inspects manifests, layout, and toolchain configs with its read-only tools, then writes a concise AGENTS.md — or, if one exists, merge-updates it preserving your content. Optional notes...' },
        { keys: '/cron', desc: 'Manage scheduled tasks (list, add/create, edit, pause, resume, run, remove)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/suggestions [accept|dismiss N|catalog|clear] (alias: /suggest)', desc: 'Review suggested automations', usage: 'Use /suggestions to list pending suggestions, /suggestions accept <id> to create the proposed automation, /suggestions dismiss <id> to reject one, /suggestions catalog to add curated starter...' },
        { keys: '/blueprint [name] [slot=value ...] (alias: /bp)', desc: 'Set up an automation from a blueprint template', usage: 'Bare /blueprint lists the catalog; /blueprint <name> starts a guided slot-filling flow on the next agent turn; /blueprint <name> slot=value ... creates the job directly.' },
        { keys: '/curator', desc: 'Background skill maintenance — status, run, pin, archive', usage: 'See Curator.' },
        { keys: '/kanban <action>', desc: 'Drive the multi-profile, multi-project collaboration board without leaving chat', usage: 'Full hermes kanban surface is available: /kanban list, /kanban show t_abc, /kanban create "title" --assignee X, /kanban comment t_abc "text", /kanban unblock t_abc, /kanban dispatch, etc....' },
        { keys: '/reload-mcp (alias: /reload_mcp)', desc: 'Reload MCP servers from config.yaml', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/reload-skills (alias: /reload_skills)', desc: 'Re-scan ~/.hermes/skills/ for newly installed or removed skills', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/reload', desc: 'Reload .env variables into the running session (picks up new API keys without restarting)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/plugins', desc: 'List installed plugins and their status', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/pet [list|<slug>]', desc: 'Toggle or adopt a petdex mascot', usage: '/pet toggles the pane, /pet list shows installed pets, /pet <slug> adopts a specific one.' },
        { keys: '/hatch <description> (alias: /generate-pet)', desc: 'Generate a brand-new petdex pet from a text description, using the configured image backend (OpenRouter / Nous Portal)', usage: 'See Pets.' }
      ] },
      { name: 'Info Commands', shortcuts: [
        { keys: '/help', desc: 'Show available commands, grouped by category', usage: 'Core commands are shown by default with skill commands collapsed to a one-line count; /help skills lists all skill commands, and /help <text> filters commands (and matching skills) by substring.' },
        { keys: '/palette', desc: 'Open the fuzzy command palette (also Ctrl+P) — type to filter all commands + skills, ↑/↓ to move, Enter to insert...', usage: 'the selected command into the composer (never auto-runs), Esc to cancel Matching is ranked by command name first, so a short query stays precise.' },
        { keys: '/version', desc: 'Show Hermes Agent version, build, and environment info', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/whoami', desc: 'Show your slash command access level (admin / user)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/usage', desc: 'Show token usage, cost breakdown, session duration, and — when available from the active provider — an Account...', usage: 'limits section with remaining quota / credits / plan usage pulled live from the provider\'s API' },
        { keys: '/topup', desc: 'Show your Nous balance and manage billing on the portal (replaces the old /credits and /billing commands)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/subscription (alias: /upgrade)', desc: 'CLI only', usage: 'View your Nous plan and change it in the browser.' },
        { keys: '/insights', desc: 'Show usage insights and analytics (last 30 days)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/update', desc: 'Update Hermes Agent to the latest version', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/platforms (alias: /gateway)', desc: 'Show gateway/messaging platform status (CLI-only summary view)', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/paste', desc: 'Attach a clipboard image', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/copy [number]', desc: 'Copy the last assistant response to clipboard (or the Nth-from-last with a number)', usage: 'CLI-only.' },
        { keys: '/image <path>', desc: 'Attach a local image file for your next prompt', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/debug', desc: 'Upload debug report (system info + logs) and get shareable links', usage: 'Also available in messaging.' },
        { keys: '/update', desc: 'Update Hermes Agent to the latest version', usage: 'Interactive CLI slash command; most also work over messaging channels.' },
        { keys: '/profile', desc: 'Show active profile name and home directory', usage: 'Interactive CLI slash command; most also work over messaging channels.' }
      ] },
      { name: 'Messaging-Only Commands', shortcuts: [
        { keys: '/start', desc: 'Platform-protocol command', usage: 'Many chat platforms (Telegram, Discord, …) send /start automatically the first time a user opens a bot conversation. Hermes acknowledges the ping silently — no agent reply, no session burn — so...' },
        { keys: '/sethome (alias: /set-home)', desc: 'Mark the current chat as the platform home channel for deliveries', usage: 'Messaging-channel slash command (Telegram, Discord, WhatsApp and other connected surfaces).' },
        { keys: '/topic [off|help|session-id]', desc: 'Telegram DM only', usage: 'Manage user-managed multi-session topic mode. /topic enables it or shows status; /topic off disables it and clears bindings; /topic help shows usage; /topic <session-id> inside a topic restores a...' },
        { keys: '/platform <list|pause|resume> [name]', desc: 'Operate a running gateway platform right from chat', usage: '/platform list shows every adapter and its state (running, paused-by-breaker, manually-paused); /platform pause <name> stops dispatching new messages to that adapter without unloading it;...' },
        { keys: '/commands [page]', desc: 'Browse all commands and skills (paginated)', usage: 'Messaging-channel slash command (Telegram, Discord, WhatsApp and other connected surfaces).' },
        { keys: '/approve [session|always]', desc: 'Approve and execute a pending dangerous command', usage: 'session approves for this session only; always adds to permanent allowlist.' },
        { keys: '/deny', desc: 'Reject a pending dangerous command', usage: 'Messaging-channel slash command (Telegram, Discord, WhatsApp and other connected surfaces).' },
        { keys: '/restart', desc: 'Gracefully restart the gateway after draining active runs', usage: 'When the gateway comes back online, it sends a confirmation to the requester\'s chat/thread.' },
        { keys: '/<skill-name>', desc: 'Invoke any installed skill by name', usage: 'Messaging-channel slash command (Telegram, Discord, WhatsApp and other connected surfaces).' }
      ] },
      { name: 'Hermes 4 Prompt Format Tokens', shortcuts: [
        { keys: '<|start_header_id|>role<|end_header_id|>', desc: 'Role header opening a system, user, or assistant turn', usage: 'Hermes 4 70B/405B use the Llama-3-Chat format; smaller variants use ChatML via the bundled chat template. Source: NousResearch/Hermes-4-405B model card.' },
        { keys: '<|eot_id|>', desc: 'End-of-turn tag closing each message', usage: 'Terminates every role block in the Llama-3-Chat prompt format.' },
        { keys: 'thinking=True', desc: 'Chat template flag that activates reasoning mode', usage: 'Pass through the tokenizer chat template; equivalent to using the deep thinking system prompt.' },
        { keys: '<think> </think>', desc: 'Tags wrapping the model\'s internal reasoning before the final answer', usage: 'Emitted at the start of the assistant turn when the model deliberates; strip or keep via template options.' },
        { keys: 'keep_cots', desc: 'Template flag controlling whether prior chain-of-thought is kept in context', usage: 'Governs the content between think tags across turns.' },
        { keys: '<tools> </tools>', desc: 'System-message block holding JSON tool definitions', usage: 'Alternatively place definitions in the messages "tools:" field and let the chat template render them.' },
        { keys: '<tool_call> </tool_call>', desc: 'Tags the model emits around a JSON function call', usage: 'One JSON object per call; tool calls can interleave with reasoning inside a single assistant turn.' },
        { keys: '<tool_response> </tool_response>', desc: 'Tags wrapping the tool result you feed back', usage: 'After the response, the model continues reasoning and produces the final answer.' },
        { keys: 'temperature=0.6, top_p=0.95, top_k=20', desc: 'Recommended sampling defaults', usage: 'Stated inference defaults for Hermes 4 in the model card.' },
        { keys: 'add_generation_prompt=True', desc: 'Template argument appending the assistant header for generation', usage: 'Use with the HF chat template instead of hand-building headers.' }
      ] }
    ]
  }
};
